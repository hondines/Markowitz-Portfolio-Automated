...
display(portfolioDailyReturn)

#Adjusting DF size
if len(portfolioDailyReturn) > len(ibovlogReturns):
  dif = len(portfolioDailyReturn) - len(ibovlogReturns)
  stock = portfolioDailyReturn.drop(portfolioDailyReturn.index[len(portfolioDailyReturn)-dif:len(portfolioDailyReturn)])
elif len(portfolioDailyReturn) < len(ibovlogReturns):
  dif = len(portfolioDailyReturn) - len(ibovlogReturns)
  ibov = ibovlogReturns.drop(ibovlogReturns.index[len(ibovlogReturns)-dif:len(ibovlogReturns)])
else:
  print("error")

len(portfolioDailyReturn)

len(ibovlogReturns)

#Plotting the Graph
x = portfolioDailyReturn["Date"]

y1 = portfolioDailyReturn["Portfolio Return"] 
y2 = ibovlogReturns["Adj Close"]
...
portfolioDailyReturn.index

nominal.associations(portfolioDailyReturn, figsize=(20,10),mark_columns=True)

#remover colunas data e return
#tentar fazer a tabela de correlção por setor
#organizar melhor os graficos

portoptWeights = pd.DataFrame(cleaned_weights, index = cleaned_weights).head(1)

portoptReturns = pd.DataFrame()
portoptReturns['Returns'] = (cumreturns*portoptWeights).sum(axis=1)
...
df[stock] = web.DataReader("ABEV3.SA", data_source='yahoo', start = startDate, end = endDate)['Adj Close']
